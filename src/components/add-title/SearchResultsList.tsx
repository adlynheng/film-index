...
export function SearchResultsList({ results, onPick }: SearchResultsListProps) {
  ...
  return (
    ...
    <ul className="absolute left-0 right-0 top-full z-[5] max-h-[250px] overflow-y-auto border border-t-0 border-border bg-paper">
      {results.map((result, index) => (
        <li key={`${result.title}-${result.year}-${index}`} className="border-b border-borderFaint">
          <button
            type="button"
